  ...
  class Context
    ...
    def get_source_code(lang, stream)
      ...
      when :c
        ...
        erbout << "MARK_FUNC_AS_USED\nstatic pcc_bool_t pcc_apply_rule(pcc_context_t *ctx, pcc_rule_t rule, pcc_thunk_array_t *thunks, pcc_value_t *value) {\n    static pcc_value_t null;\n    pcc_thunk_chunk_t *c = NULL;\n    const size_t p = ctx->buffer_start_position + ctx->position_offset;\n".freeze

        if @location
          erbout << "    const pcc_location_t p_loc = pcc_location_add(ctx->buffer_start_position_loc, ctx->position_offset_loc);\n".freeze
        end
        erbout << "    pcc_bool_t b = PCC_TRUE;\n    pcc_lr_answer_t *a = pcc_lr_table__get_answer(ctx, &ctx->lrtable, p, rule);\n    pcc_lr_head_t *h = pcc_lr_table__get_head(ctx, &ctx->lrtable, p);\n    if (h != NULL) {\n        if (a == NULL && rule != h->rule && pcc_rule_set__index(ctx->auxil, &h->invol, rule) == PCC_VOID_VALUE) {\n            b = PCC_FALSE;\n            c = NULL;\n        }\n        else if (pcc_rule_set__remove(ctx->auxil, &h->eval, rule)) {\n            b = PCC_FALSE;\n            c = rule(ctx);\n            a = pcc_lr_answer__create(ctx, PCC_LR_ANSWER_CHUNK, ctx->buffer_start_position + ctx->position_offset".freeze

        if @location
          erbout << ", pcc_location_add(ctx->buffer_start_position_loc, ctx->position_offset_loc)".freeze
        end
        erbout << ");\n            a->data.chunk = c;\n            pcc_lr_table__hold_answer(ctx, &ctx->lrtable, p, a);\n        }\n    }\n    if (b) {\n        if (a != NULL) {\n            ctx->position_offset = a->pos - ctx->buffer_start_position;\n".freeze

        if @location
          erbout << "            ctx->position_offset_loc = pcc_location_sub(a->pos_loc, ctx->buffer_start_position_loc);\n".freeze
        end
        ...
        erbout << ");\n            a->data.lr = e;\n            pcc_lr_table__set_answer(ctx, &ctx->lrtable, p, rule, a);\n            c = rule(ctx);\n            pcc_lr_stack__pop(ctx->auxil, &ctx->lrstack);\n            a->pos = ctx->buffer_start_position + ctx->position_offset;\n".freeze

        if @location
          erbout << "            a->pos_loc = pcc_location_add(ctx->buffer_start_position_loc, ctx->position_offset_loc);\n".freeze
        end
        ...
        erbout << ");\n                    a->data.chunk = c;\n                    pcc_lr_table__hold_answer(ctx, &ctx->lrtable, p, a);\n                }\n                else {\n                    pcc_lr_answer__set_chunk(ctx, a, a->data.lr->seed);\n                    if (a->data.chunk == NULL) {\n                        c = NULL;\n                    }\n                    else {\n                        pcc_lr_table__set_head(ctx, &ctx->lrtable, p, h);\n                        for (;;) {\n                            ctx->position_offset = p - ctx->buffer_start_position;\n".freeze

        if @location
          erbout << "                            ctx->position_offset_loc = pcc_location_sub(p_loc, ctx->buffer_start_position_loc);\n".freeze
        end
        erbout << "                            pcc_rule_set__copy(ctx->auxil, &h->eval, &h->invol);\n                            c = rule(ctx);\n                            if (c == NULL || ctx->buffer_start_position + ctx->position_offset <= a->pos) break;\n                            pcc_lr_answer__set_chunk(ctx, a, c);\n                            a->pos = ctx->buffer_start_position + ctx->position_offset;\n".freeze
